...
such as local field tensors, from quantum chemical data (e.g., electron density).
此脚本概述了从量子化学数据（例如，电子密度）生成原子张量（例如，局部场张量）的并行化框架。
...
def generate_atomic_tensors_parallel_demo(args):
    ...
    if world.rank == 0:
        ...
        print(f"[{base_name}] === DEMO: Starting Parallel Local Field Tensor Generation ===")
    ...
    if world.rank == 0:
        ...
        if all_tensors:
            ...
            output_csv = output_dir / f'{base_name}-local-field-tensors-demo.csv'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="DEMO FRAMEWORK for parallel generation of local field tensors from quantum chemical data. (从量子化学数据并行生成局部场张量的演示框架)",
        formatter_class=argparse.argparse.ArgumentDefaultsHelpFormatter
    )
